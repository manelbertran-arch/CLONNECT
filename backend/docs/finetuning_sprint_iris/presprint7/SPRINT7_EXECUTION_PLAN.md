...
**Patch v1.2 (2026-04-26):** Smoke training descubrió que B1 (Opción C §5) era incompleto. El `response_part` era correcto pero faltaba el prepend manual de `CHANNEL_PREFIX`. Fix correcto en commit `e38b9a77` — el prepend debe ocurrir en una función SEPARADA (`add_channel_prefix`) aplicada ANTES de `standardize_data_formats`, ya que después del standardize el formato cambia de `role/content` a `from/value`. También: `MAX_SEQ_LENGTH` 2048 → 4096 (Doc D ≈ 2044 tokens excede 2048 con cualquier turno). Ver nota "Opción C — Parte 2" en §5.
