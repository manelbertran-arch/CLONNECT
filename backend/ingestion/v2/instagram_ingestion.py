...
class InstagramIngestionV2:
    ...
    async def _scrape_instagram(self, username: str, max_posts: int) -> List:
        ...
        # OPCIÓN 2: Instaloader fallback — DISABLED
        # Instaloader is a public scraper that violates Instagram ToS and triggers
        # account-level rate limits (429) that can result in permanent blocks.
        # When Meta API fails (wrong token/page_id), return empty gracefully.
        logger.warning(
            f"[Instagram] Meta API failed for @{username} and Instaloader fallback is disabled. "
            "Skipping post ingestion. Fix the instagram_token/page_id to restore."
        )
        return []
