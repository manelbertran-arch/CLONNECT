            ...
            if stats:
                return {
                    ...
                    "metrics": {
                        ...
                        "total_followers": stats.get("total_leads", 0),
                        "leads": stats.get("total_leads", 0),
                        "customers": 0,
                        "high_intent_followers": stats.get("hot_leads", 0),
                        "conversion_rate": 0.0,
                    },
                }
                    ...
                    if creator:
                        ...
                        if lead:
                            ...
                            return {
                                ...
                                "is_lead": True,
                                "is_customer": (
                                    lead.context.get("is_customer", False)
                                    if lead.context
                                    else False
                                ),
            ...
            return {
                ...
                "platform": (
                    ...
                    if follower_id.startswith("ig_")
                        ...
                        else "whatsapp" if follower_id.startswith("wa_") else "instagram"
                    )
                ),
...
async def update_follower_status(creator_id: str, follower_id: str, data: dict = Body(...)):
                ...
                try:
                    ...
                    if creator:
                        ...
                        try:
                            lead = (
                                session.query(Lead)
                                .filter_by(creator_id=creator.id, id=uuid.UUID(follower_id))
                                .first()
                            )
                        ...
                        if lead:
                            ...
                            session.commit()
                            logger.info(
                                f"Updated lead {follower_id} status to {db_status} (intent: {new_intent})"
                            )
                            return {
                                "status": "ok",
                                "follower_id": follower_id,
    ...
    try:
        ...
        from api.services.data_sync import STORAGE_PATH, _load_json
...
async def get_conversations(creator_id: str, limit: int = 50, offset: int = 0):
    ...
    try:
        ...
        if os.path.exists(creator_dir):
            all_files = sorted(
                os.listdir(creator_dir),
                key=lambda x: os.path.getmtime(os.path.join(creator_dir, x)),
                reverse=True,
            )
                    ...
                    all_conversations.append(
                        {
                            ...
                            "platform": (
                                "instagram"
                                if follower_id.startswith("ig_")
                                else "telegram" if follower_id.startswith("tg_") else "instagram"
                            ),
                            "total_messages": user_msgs,
            ...
            return {
                ...
                "limit": limit,
                "offset": offset,
                "has_more": offset + len(all_conversations) < total_count,
                "product_price": product_price,
            }
    except Exception as e:
