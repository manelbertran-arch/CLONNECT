...
class DetectedContext:
    ...
    def build_context_notes(self) -> List[str]:
        ...
        Notes are in English (LLM lingua franca) regardless of creator
        dialect — the LLM handles language adaptation via the personality
        prompt. Mixing Spanish notes into an English creator's context
        biases the LLM toward Spanish responses.
        """
        notes = []
        if self.is_b2b:
            note = "Lead appears to represent a company/brand"
        ...
        if self.is_meta_message:
            notes.append("Lead is referencing earlier messages.")
        if self.is_correction:
            notes.append("Lead is correcting a misunderstanding.")
        if self.objection_type:
            notes.append(f"Lead has a {self.objection_type} objection.")
