...
def import_messages(conn, lead_id: str, messages: list, existing_fingerprints: set) -> tuple:
    ...
    for msg in messages:
        ...
        messages_to_insert.append((msg_id, lead_id, role, content, intent, created_at, status))
...
def main():
    ...
    try:
        ...
        for i, conv in enumerate(conversations):
            ...
            if (i + 1) % 10 == 0:
                print(
                    f"  Progress: {i+1}/{len(conversations)} conversations, {stats['messages_imported']} imported, {stats['messages_skipped']} skipped"
                )
