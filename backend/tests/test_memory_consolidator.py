...
Tests:
  ...
  - Phase 3: Consolidate (LLM-powered + algorithmic fallback)
...
class TestNearDuplicates:
    ...
    def test_exact_duplicates(self):
        facts = [
            make_fact(fact_text="Le gusta el yoga por las mañanas"),
            make_fact(fact_text="Le gusta el yoga por las mañanas"),
        ]
        dupes = _find_near_duplicates(facts)
        assert len(dupes) == 1
    ...
    def test_near_duplicates(self):
        facts = [
            make_fact(fact_text="Le gusta el yoga por las mañanas y la meditación"),
            make_fact(fact_text="Le gusta el yoga por las mañanas y la meditación guiada"),
        ]
        dupes = _find_near_duplicates(facts)
        assert len(dupes) >= 1
...
class TestConsolidateCreator:
    ...
    async def test_dedup_and_compress_flow(self):
        """Leads with duplicates get deduped and compressed."""
        lead_id = str(uuid.uuid4())
