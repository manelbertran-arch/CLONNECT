"""
CPE Ablation — Layer 2 + System #10: Episodic Memory (conversation_embeddings).
...
ADDS System #10:
  - Searches conversation_embeddings (pgvector) for past messages relevant
    to the current lead message
  - Appends up to 3 relevant past exchanges to the system prompt
  - Uses _episodic_search() from core/dm/phases/context.py
  - Gated: only fires for messages with 15+ chars and 3+ unique words
  - Deduplicates against recent history (multi-turn turns)

Methodology:
  - PersonaGym (EMNLP 2025) + AbGen (ACL 2025)
  - 3 runs × 50 cases = 150 observations
  - L1 (9) + L2 (5+SemSim) + L3 (BERTScore + rep + hallucination)
  - Wilcoxon signed-rank + Cliff's delta vs Layer 2

Usage:
    railway run python3 tests/cpe_ablation_layer2_plus_sys10.py --creator iris_bertran
    railway run python3 tests/cpe_ablation_layer2_plus_sys10.py --creator iris_bertran --evaluate-only

Output:
    tests/cpe_data/{creator}/sweep/layer2_plus_system10_run{N}_{ts}.json
    tests/cpe_data/{creator}/sweep/layer2_plus_system10.json
"""
...
logger = logging.getLogger("cpe_l2_sys10")
logger.setLevel(logging.INFO)

# Import reusable components from Layer 2 script
...
# Import SemSim from sys06 script
from tests.cpe_ablation_layer2_plus_sys06 import (
    compute_semsim,
    load_layer2_per_case,
)

ABLATION_NAME = "layer2_plus_system10"
SYSTEMS_ACTIVE = [
...
# =============================================================================
# SYSTEM #10: Episodic Memory Search
# =============================================================================

def _get_raw_connection():
    """Get a raw psycopg2 connection, bypassing SQLAlchemy ORM."""
    import psycopg2
...
def _resolve_creator_uuid(conn, creator_slug: str) -> str:
    """Resolve creator slug to UUID using raw SQL."""
    with conn.cursor() as cur:
        cur.execute("SELECT id FROM creators WHERE name = %s LIMIT 1", (creator_slug,))
        row = cur.fetchone()
        return str(row[0]) if row else ""


_st_model = None
_corpus_cache: Dict[str, list] = {}  # creator_uuid → [(content, role)]


def _get_st_model():
    """Lazy-load SentenceTransformer (same model used for SemSim)."""
    global _st_model
...
def _load_corpus(conn, creator_uuid: str, max_rows: int = 2000) -> list:
    """Load recent conversation_embeddings texts for local semantic search."""
    if creator_uuid in _corpus_cache:
        return _corpus_cache[creator_uuid]
    try:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                LIMIT %s
            """, (creator_uuid, max_rows))
            rows = cur.fetchall()
        corpus = [(r[0], r[1]) for r in rows]
        _corpus_cache[creator_uuid] = corpus
        return corpus
    except Exception as e:
        logger.debug("Corpus load failed: %s", e)
        return []


def episodic_search_for_case(
    creator_id: str,
    message: str,
    ...
    """Run episodic memory search using local SentenceTransformer.

    Bypasses OpenAI embeddings (quota issue) by using paraphrase-multilingual-MiniLM
    for both query and corpus encoding. Searches over text from conversation_embeddings.
    """
    import numpy as np

    _MIN_SIM = 0.55  # Local model threshold (calibrated lower than OpenAI)
    _MAX_RESULTS = 3
    _MAX_CONTENT_CHARS = 250

    # Gate: only for messages with 15+ chars and 3+ unique words (O4)
    msg_stripped = message.strip()
    msg_words = set(msg_stripped.lower().split())
    if len(msg_stripped) < 15 or len(msg_words) < 3:
        return ""

    if not _conn or not _creator_uuid:
        return ""

    try:
        model = _get_st_model()
        corpus = _load_corpus(_conn, _creator_uuid)
        if not corpus:
            return ""

        # Encode query
        query_emb = model.encode([message], show_progress_bar=False)[0]

        # Encode corpus (cached after first call per creator)
        cache_key = f"emb_{_creator_uuid}"
        if cache_key not in _corpus_cache:
            texts = [c[0] for c in corpus]
            _corpus_cache[cache_key] = model.encode(texts, show_progress_bar=False, batch_size=64)
        ...
        top_idxs = np.argsort(sims)[::-1][:_MAX_RESULTS * 2]
        results = [
            (corpus[i][0], corpus[i][1], float(sims[i]))
            for i in top_idxs
            if sims[i] >= _MIN_SIM
        ]
        ...
        if recent_history:
            ...
            results = [r for r in results if r[0][:100] not in recent_contents]
        ...
        lines = []
        for content, role, sim in results:
            role_label = "lead" if role == "user" else "tú"
        ...
        logger.debug("Episodic search local failed: %s", e)
        return ""


# =============================================================================
# GENERATION — Layer 2 + System #10
# =============================================================================

async def generate_l2_sys10_run(
    ...
    """
    One run through Layer 2 + System #10 (episodic memory).
    Identical to Layer 2 except episodic context is appended to system prompt.
    """
    ...
    logger.info("[Run %d] l2+sys10 | model=%s | cases=%d", run_idx, model, len(test_cases))
    ...
    for i, tc in enumerate(test_cases, 1):
        ...
        # System #10: Episodic memory search (before guards, so we can annotate)
        recent_turns = tc.get("turns", [])
        recent_history = [
        ...
        try:
            episodic_ctx = await asyncio.to_thread(
                episodic_search_for_case,
                creator_id, original_message, recent_history,
                db_conn, creator_uuid,
            )
            if episodic_ctx:
                counts["episodic_recalled"] += 1
                counts["episodic_chars_total"] += len(episodic_ctx)
        except Exception as e:
            logger.debug("Episodic search error case %d: %s", i, e)
        ...
        if guard_flags["is_empty"]:
            ...
            if not bot_response and len(message.strip()) <= POOL_MAX_MSG_LEN:
                try:
                    ...
                    match = variator.try_pool_response(
                        ...
                        turn_index     = i,
                        conv_id        = f"l2s10_run{run_idx}_{i}",
            ...
            try:
                frust_signals, frust_score = frustration_detector.analyze_message(
                    message, f"l2s10_run{run_idx}_{i}"
                )
            ...
            # ── LLM GENERATION with episodic context (System #10) ──────────
            if not bot_response:
                counts["llm_calls"] += 1
                ...
                # System #10: Append episodic memory to system prompt
                if episodic_ctx:
                    prompt = prompt + "\n\n" + episodic_ctx
        ...
        results.append({
            ...
            "episodic_context": episodic_ctx[:200] if episodic_ctx else "",
            "episodic_chars":   len(episodic_ctx),
        ...
        if i % 10 == 0 or not bot_response:
            tag = source.upper()[:5]
            ep  = f"ep={len(episodic_ctx)}ch" if episodic_ctx else "ep=0"
            print(f"  [{tag:5}] [{i:02d}/{len(test_cases)}] {tc['id']} ({ep}): {bot_response[:55]!r}")
    ...
    ep_n   = sum(1 for r in results if r["episodic_chars"] > 0)
    ep_avg = statistics.mean(r["episodic_chars"] for r in results) if results else 0
    print(
        ...
        f"episodic={ep_n}/{len(results)} avg_chars={ep_avg:.0f} | avg {avg_ms:.0f}ms"
    )
...
def _print_report(data: dict, naked_bert: float, layer1_bert: float, layer2_bert: float) -> None:
    ...
    print(f"\n{'='*80}")
    print(f"ABLATION REPORT — Layer 2 + System #10: Episodic Memory")
    ...
    intercept_labels = [
        ...
        ("episodic_recalled",        "#10 Episodic recalled"),
        ("episodic_chars_total",     "#10 Episodic chars total"),
    ]
    ...
    print(f"\n  {'METRIC':<22} {'NAKED':>8} {'L1':>8} {'L2':>8} {'L2+S10':>8}  "
    ...
    # Episodic memory coverage stats
    ep_stats = data.get("episodic_stats", {})
    if ep_stats:
        print(f"\n  EPISODIC MEMORY STATS:")
        print(f"    Cases with episodic context: {ep_stats.get('recalled_pct', 0):.1f}%")
        print(f"    Avg chars when recalled:     {ep_stats.get('avg_chars_recalled', 0):.0f}")
        print(f"    Gated (too short):           {ep_stats.get('gated_pct', 0):.1f}%")
    ...
    for c in data["sample_cases"]:
        ...
        print(f"\n  Case {c['case_idx']} [{src_label}] [{c['category']}/{c['language']}] (ep={ep_ch}ch)")
        print(f"  Signals: {sig_str}")
        if c.get("episodic_context"):
            print(f"  Episodic: \"{c['episodic_context'][:150]}\"")
...
async def main():
    parser = argparse.ArgumentParser(description="CPE Ablation — Layer 2 + System #10: Episodic Memory")
    ...
    print("\n" + "="*80)
    print("LAYER 2 + SYSTEM #10 ABLATION — Episodic Memory (conversation_embeddings)")
    print("="*80)
    ...
    # Establish raw DB connection for episodic search (bypasses SQLAlchemy ORM)
    db_conn = None
    creator_uuid = ""
    db_url = os.getenv("DATABASE_URL")
    if db_url and not args.evaluate_only:
        try:
            db_conn = _get_raw_connection()
            creator_uuid = _resolve_creator_uuid(db_conn, creator_id)
            if creator_uuid:
                # Count available embeddings
                with db_conn.cursor() as cur:
                    cur.execute(
                        "SELECT COUNT(*) FROM conversation_embeddings WHERE creator_id = %s",
                        (creator_uuid,),
                    )
                    n_emb = cur.fetchone()[0]
                print(f"DB connected: creator_uuid={creator_uuid[:8]}... | {n_emb} embeddings")
            else:
                print(f"WARNING: Creator '{creator_id}' not found in DB — episodic search will be empty.")
        except Exception as e:
            print(f"WARNING: DB connection failed: {e}")
            db_conn = None
    elif not args.evaluate_only:
        print("WARNING: DATABASE_URL not set — episodic search will return empty for all cases.")
        print("         Run via `railway run` for real episodic context from conversation_embeddings.")
    ...
    if args.evaluate_only:
        ...
        for run_idx in range(1, n_runs + 1):
            ...
            run_results, counts = await generate_l2_sys10_run(
                conversations, base_prompt, variator, calibration,
                creator_id, model, run_idx, args.delay,
                db_conn=db_conn, creator_uuid=creator_uuid,
            )
    ...
    for rf in run_files:
        ...
        ep_n     = sum(1 for r in results if r.get("episodic_chars", 0) > 0)
        ...
        print(f"  Run {data['run']}: L1={l1['score']}  "
              ...
              f"[llm={llm_n} pool={pool_n} crisis={crisis_n} ep={ep_n}]")
    ...
    for run_data in all_run_results:
        for r in run_data:
            all_ep_chars.append(r.get("episodic_chars", 0))
    recalled = [c for c in all_ep_chars if c > 0]
    gated    = [c for c in all_ep_chars if c == 0]
    ep_stats = {
        "recalled_pct":       round(len(recalled) / len(all_ep_chars) * 100, 1) if all_ep_chars else 0,
        "avg_chars_recalled":  round(statistics.mean(recalled), 0) if recalled else 0,
        "gated_pct":          round(len(gated) / len(all_ep_chars) * 100, 1) if all_ep_chars else 0,
        "total_cases":        len(all_ep_chars),
    }
    ...
    # Prioritize cases WITH episodic context (the interesting ones for System #10)
    ep_cases     = [r for r in r1_text if r.get("episodic_chars", 0) > 0]
    no_ep_cases  = [r for r in r1_text if r.get("episodic_chars", 0) == 0 and r.get("source") == "llm"]
    ...
    # Pick 3 with episodic context, 1 without, 1 pool/other
    for r in (_pick(ep_cases, 3) + _pick(no_ep_cases) + _pick(pool_cases) +
              _pick(frust_cases)):
    ...
    for idx, r in enumerate(sample, 1):
        sample_cases.append({
            ...
            "episodic_context":  r.get("episodic_context", ""),
            "episodic_chars":    r.get("episodic_chars", 0),
            "source":            r.get("source", ""),
