"""
CPE Ablation — Layer 2: Doc D v3 + Full Fase 1 Detection Pipeline.

Systems active (in order):
...
Systems OFF:
  ✗ Memory / RAG / few-shot / style normalizer / post-processing

Methodology:
  - PersonaGym (EMNLP 2025) + AbGen (ACL 2025)
  - 3 runs × 50 cases = 150 observations
  - L1 (9) + L2 (5) + L3 (BERTScore + rep + hallucination)
  - Wilcoxon + Cliff's delta vs NAKED and vs DOC D v3 only (Layer 1)

Usage:
    railway run python3 tests/cpe_ablation_layer2_full_detection.py --creator iris_bertran
    railway run python3 tests/cpe_ablation_layer2_full_detection.py --creator iris_bertran --evaluate-only

Output:
    tests/cpe_data/{creator}/sweep/layer2_full_detection_run{N}_{ts}.json
    tests/cpe_data/{creator}/sweep/layer2_full_detection.json
"""
...
logger = logging.getLogger("cpe_layer2_full")
...
def _is_text_ground_truth(gt: str) -> bool:
    ...
    # Exact match against known media placeholders
    if gt_lower in _MEDIA_PLACEHOLDERS:
        return False
    # Catch bracket-wrapped media tags like [audio], [sticker], [📷 photo], etc.
...
def run_input_guards(message: str) -> Tuple[str, dict]:
    """
    Run all input guards. Returns (processed_message, guard_flags).
    guard_flags keys: is_empty, is_truncated, injection_flagged, is_media.
    """
    flags: dict = {
        "is_empty":         False,
        "is_truncated":     False,
...
# =============================================================================
# SYSTEM PROMPT AUGMENTATION based on detection results
# =============================================================================

def build_augmented_prompt(base_prompt: str, frustration_level: int,
                            context_signals) -> str:
    """Append detection-driven notes to the base Doc D system prompt."""
    notes = []
...
# =============================================================================
# METRICS  (verbatim from layer1 / docd_v3_plus_pool scripts)
# =============================================================================
...
# =============================================================================
# LOAD PER-CASE SCORES from existing run files
# =============================================================================
...
def load_naked_per_case(sweep_dir: Path) -> Dict[str, List[float]]:
    files = sorted(sweep_dir.glob("naked_zero_run*_20260331_193231.json"))
    if not files:
        files = sorted(sweep_dir.glob("naked_zero_run*.json"))
...
def load_layer1_per_case(sweep_dir: Path) -> Dict[str, List[float]]:
    ...
    if not complete:
        logger.warning("No complete layer1_doc_d 3-run set — stat comparison vs L1 skipped")
        return {}
    ts, files = complete[0]
    logger.info("Layer1 baseline: %s (%d files)", ts, len(files))
    return _extract_per_case_from_files(files)
...
# =============================================================================
# GENERATION — Layer 2: full detection pipeline
# =============================================================================

async def generate_layer2_run(
    ...
    """
    One run through the full Fase 1 detection pipeline.
    ...
    counts = defaultdict(int)   # intercept / annotation counts

    logger.info("[Run %d] layer2_full | model=%s | cases=%d", run_idx, model, len(test_cases))
    ...
    for i, tc in enumerate(test_cases, 1):
        ...
        if guard_flags["is_empty"]:
            ...
            # Produce empty response — LLM would also produce nothing useful
            bot_response = ""
        else:
            ...
            try:
                ...
                if sensitive and sensitive.confidence >= SENSITIVE_CONFIDENCE:
                    ...
                    if sensitive.confidence >= SENSITIVE_ESCALATION:
                        # Crisis escalation — always Spanish for iris_bertran
            ...
            if not bot_response and len(message.strip()) <= POOL_MAX_MSG_LEN:
                try:
                    ...
                    match = variator.try_pool_response(
                        ...
                        turn_index    = i,
                        conv_id       = f"l2_ablation_run{run_idx}_{i}",
        ...
        results.append({
            ...
            "run":           run_idx,
            # Detection metadata
            "source":           source,          # "llm" | "pool" | "crisis" | "empty_skip"
    ...
    print(
        ...
        f"crisis={counts['sensitive_escalated']} | avg {avg_ms:.0f}ms"
    )
...
async def main():
    parser = argparse.ArgumentParser(description="CPE Ablation — Layer 2: Full Detection Pipeline")
    ...
    print("\n" + "="*74)
    print("LAYER 2 ABLATION — Doc D v3 + Full Fase 1 Detection")
    print("="*74)
    ...
    if args.evaluate_only:
        run_files = sorted(sweep_dir.glob("layer2_full_detection_run*.json"))[:n_runs]
        if not run_files:
            print("ERROR: --evaluate-only but no layer2_full_detection_run*.json found.")
            sys.exit(1)
        ...
        for run_idx in range(1, n_runs + 1):
            ...
            run_results, counts = await generate_layer2_run(
                conversations, base_prompt, variator, calibration,
                creator_id, model, run_idx, args.delay,
            )
            all_counts.append(counts)

            rf = sweep_dir / f"layer2_full_detection_run{run_idx}_{ts}.json"
            payload = {
                "ablation":        "layer2_full_detection",
                "creator":         creator_id,
                "model":           model,
    ...
    crisis_cases  = [r for r in r1_text if r.get("source") == "crisis"]
    media_cases   = [r for r in r1_text if r.get("guard_flags", {}).get("is_media")]
    edge_cases    = crisis_cases or media_cases or [r for r in r1_text if r.get("guard_flags", {}).get("injection_flagged")]

    sample: List[Dict] = []
    def _pick(pool: List, n: int = 1) -> List:
        return random.sample(pool, min(n, len(pool))) if pool else []

    for r in (_pick(pool_cases) + _pick(frust_cases) + _pick(normal_cases) +
              _pick(ctx_cases) + _pick(edge_cases)):
    ...
    out_path = sweep_dir / "layer2_full_detection.json"
    out_path.write_text(json.dumps(final, indent=2, ensure_ascii=False))
    print(f"\nSaved → {out_path}")

    _print_report(final, naked_bert, layer1_bert)
...
def _print_report(data: dict, naked_bert: float, layer1_bert: float) -> None:
    ...
    print(f"\n{'='*76}")
    print(f"ABLATION REPORT — Layer 2: Full Fase 1 Detection Pipeline")
    ...
    print(f"\n  {'METRIC':<22} {'NAKED':>8} {'L1(DocD)':>8} {'L2(Full)':>8}  "
          f"{'vs-naked p':>10}  {'vs-L1 p':>8}  {'Cliff-L1':>9}  {'Sig-L1':>6}")
    print(f"{'─'*102}")
    ...
    for label, key in DISPLAY:
        ...
        cur     = n.get("current_mean", l1.get("current_mean", "—"))
        np_val  = n.get("p_value",  "—")
        lp_val  = l1.get("p_value",  "—")
        ld      = l1.get("cliffs_d",  "—")
        ls      = "✓" if l1.get("significant") else "·"
        ns      = "✓" if n.get("significant")  else "·"
        print(f"  {label:<22} {_fmt(n_mean)} {_fmt(l1_mean)} {_fmt(cur)} "
              f"  {_fmt(np_val)} {ns}  {_fmt(lp_val)} {ls}  {_fmt(ld)}  {l1.get('magnitude','—'):>9}")

    bert_cur = data["l3"]["agg"]["coherence_bert_f1"]["mean"]
    bert_l1_val = _fmt(layer1_bert)
    print(f"  {'BERTScore':<22} {_fmt(naked_bert)} {bert_l1_val} {_fmt(bert_cur)}")

    print(f"\n  L1 scores: {data['l1']['score_per_run']}")

    print(f"\n{'─'*76}")
    print("  5 SAMPLE CASES — diverse detection signals")
    print(f"{'─'*76}")
